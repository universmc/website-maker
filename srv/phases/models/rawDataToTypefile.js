...
import { toJson, toXml, toMarkdown, toHtml } from './models/rawDataToTypeFile.js';
...
function generateGanttTable(data) {
  ...
  data.forEach(phase => phase.taches.forEach(tache => tache.actions.forEach(action => {
    ...
    const rowData = [
      ...
      steps.map(step => `${step.name}: ${step.promptStep}`).join(', '),
    ...
    rowData.forEach((data, index) => {
      ...
      if (index === headers.indexOf("URL Output")) {
        createDownloadLink(cell, rowData, action.id);
      } else {
...
function createDownloadLink(cell, rowData, actionId) {
  // Créer des liens de téléchargement pour différents formats
  const jsonData = toJson(rowData);
  const xmlData = toXml(rowData);
  const markdownData = toMarkdown(rowData);
  const htmlData = toHtml(markdownData);

  // Lien pour télécharger en JSON
  const jsonLink = document.createElement('a');
  jsonLink.href = 'data:text/json;charset=utf-8,' + encodeURIComponent(jsonData);
  jsonLink.download = `rawData_${actionId}.json`;
  jsonLink.textContent = 'Download JSON';
  cell.appendChild(jsonLink);

  // Ajoutez d'autres liens pour XML, Markdown, HTML de manière similaire
  // ...
}
